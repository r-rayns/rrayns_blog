...
I've put the python script on my [GitHub](https://github.com/End-S/station_epd_watcher/blob/master/history_train.py), it is hard coded to Emsworth station but with a little effort could easily be changed to another. The only restriction is screen size, it would be difficult to fit say, Clapham Junction on a small 2.13" display, but the makers of Inky pHAT now do a 4.2" equivalent called Inky wHAT. 